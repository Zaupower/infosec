...
def save_to_influxdb(data):
    ...
    for product in data:
        ...
        point1 = Point(str(product["organization"])).tag("product", str(product["title"])).field("price", float(product["regular_price"].replace(',', '.')))
        point2 = Point(str(product["organization"])).tag("product", str(product["title"])).field("price", float(product["discounted_price"].replace(',', '.')))
...
def main():
    ...
    while True:
        products = scrape_website(url_to_scrape, "casapeixoto")
        save_to_influxdb(products)
